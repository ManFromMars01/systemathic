...
$arrayoRStsubcateg["CountryID"] = getFormSQLQuoted($objConn1, "tsubcateg", "CountryID", "txttsubcategCountryID");
...
$arrayoRStsubcateg["BranchID"] = getFormSQLQuoted($objConn1, "tsubcateg", "BranchID", "txttsubcategBranchID");
...
$arrayoRStsubcateg["CatID"] = getFormSQLQuoted($objConn1, "tsubcateg", "CatID", "txttsubcategCatID");
        if (getRequest("txttsubcategSubCatID") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>Sub Cat ID:</strong> : Required field <hr>\n";
        endif;
$arrayoRStsubcateg["SubCatID"] = getFormSQLQuoted($objConn1, "tsubcateg", "SubCatID", "txttsubcategSubCatID");
        if (getRequest("txttsubcategDescription") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>Description:</strong> : Required field <hr>\n";
        endif;
$arrayoRStsubcateg["Description"] = getFormSQLQuoted($objConn1, "tsubcateg", "Description", "txttsubcategDescription");
$tsql = $objConn1->GetUpdateSQL($oRStsubcateg, $arrayoRStsubcateg, true, get_magic_quotes_gpc());
$badsql = strpos($tsql, "UPDATE  SET");
if($badsql === false):
    $sql = $tsql;
else:
    $sql = "UPDATE " . "tsubcateg" . " SET " . substr($tsql,12);
endif;
$dbUpdate = "";
foreach ($arrayoRStsubcateg as $fld => $value) {
  $dbUpdate .= $fld . " = " . $value . ",";
}
$dbUpdate = rtrim($dbUpdate,",");
$wherePos = strpos($tsql, " WHERE");
$sql = "UPDATE " . "tsubcateg" . " SET " . $dbUpdate . substr($tsql, $wherePos);
...
if($flgMissing == true) {
  $_SESSION["Updatetsubcateg_EditFailed"] = 1;
  $_SESSION["SavedEdittsubcategCountryID"] = $_POST["txttsubcategCountryID"];
  $_SESSION["SavedEdittsubcategBranchID"] = $_POST["txttsubcategBranchID"];
  $_SESSION["SavedEdittsubcategCatID"] = $_POST["txttsubcategCatID"];
  $_SESSION["SavedEdittsubcategSubCatID"] = $_POST["txttsubcategSubCatID"];
  $_SESSION["SavedEdittsubcategDescription"] = $_POST["txttsubcategDescription"];
}
else {
