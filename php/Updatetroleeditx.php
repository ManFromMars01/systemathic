...
$sql = "SELECT trole.CountryID, trole.BranchID, trole.RoleID, trole.Description, trole.SecurityLevel  FROM  trole WHERE  trole.CountryID = '" . $ID1 . "'" . " AND trole.BranchID = '" . $ID2 . "'" . " AND trole.RoleID = '" . $ID3 . "'";
$oRStrole = $objConn1->SelectLimit($sql,1);
$myStatus = "";
$flgMissing = false;
$arrayoRStrole = array();
if (!$oRStrole):
...
$arrayoRStrole["CountryID"] = getFormSQLQuoted($objConn1, "trole", "CountryID", "txttroleCountryID");
        if (getRequest("txttroleBranchID") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>Branch ID:</strong> : Required field <hr>\n";
        endif;
$arrayoRStrole["BranchID"] = getFormSQLQuoted($objConn1, "trole", "BranchID", "txttroleBranchID");
        if (getRequest("txttroleRoleID") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>Role ID:</strong> : Required field <hr>\n";
        endif;
$arrayoRStrole["RoleID"] = getFormSQLQuoted($objConn1, "trole", "RoleID", "txttroleRoleID");
$arrayoRStrole["Description"] = getFormSQLQuoted($objConn1, "trole", "Description", "txttroleDescription");
$arrayoRStrole["SecurityLevel"] = getFormSQLQuoted($objConn1, "trole", "SecurityLevel", "txttroleSecurityLevel");
$tsql = $objConn1->GetUpdateSQL($oRStrole, $arrayoRStrole, true, get_magic_quotes_gpc());
$badsql = strpos($tsql, "UPDATE  SET");
if($badsql === false):
    $sql = $tsql;
else:
    $sql = "UPDATE " . "trole" . " SET " . substr($tsql,12);
endif;
$dbUpdate = "";
foreach ($arrayoRStrole as $fld => $value) {
  $dbUpdate .= $fld . " = " . $value . ",";
}
$dbUpdate = rtrim($dbUpdate,",");
$wherePos = strpos($tsql, " WHERE");
$sql = "UPDATE " . "trole" . " SET " . $dbUpdate . substr($tsql, $wherePos);
...
if($flgMissing == true) {
  $_SESSION["Updatetrole_EditFailed"] = 1;
  $_SESSION["SavedEdittroleCountryID"] = $_POST["txttroleCountryID"];
  $_SESSION["SavedEdittroleBranchID"] = $_POST["txttroleBranchID"];
  $_SESSION["SavedEdittroleRoleID"] = $_POST["txttroleRoleID"];
  $_SESSION["SavedEdittroleDescription"] = $_POST["txttroleDescription"];
  $_SESSION["SavedEdittroleSecurityLevel"] = $_POST["txttroleSecurityLevel"];
}
else {
  $_SESSION["Updatetrole_EditFailed"] = 0;
}
