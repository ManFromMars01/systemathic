...
$arrayoRStcalendar["CountryID"] = getFormSQLQuoted($objConn1, "tcalendar", "CountryID", "txttcalendarCountryID");
...
$arrayoRStcalendar["BranchID"] = getFormSQLQuoted($objConn1, "tcalendar", "BranchID", "txttcalendarBranchID");
        if (getRequest("txttcalendarDate") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>Date:</strong> : Required field <hr>\n";
        endif;
$arrayoRStcalendar["Date"] = getFormSQLQuoted($objConn1, "tcalendar", "Date", "txttcalendarDate");
        if (getRequest("txttcalendarDescription") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>Description:</strong> : Required field <hr>\n";
        endif;
$arrayoRStcalendar["Description"] = getFormSQLQuoted($objConn1, "tcalendar", "Description", "txttcalendarDescription");
...
$arrayoRStcalendar["Type"] = getFormSQLQuoted($objConn1, "tcalendar", "Type", "txttcalendarType");
$tsql = $objConn1->GetUpdateSQL($oRStcalendar, $arrayoRStcalendar, true, get_magic_quotes_gpc());
$badsql = strpos($tsql, "UPDATE  SET");
if($badsql === false):
    $sql = $tsql;
else:
    $sql = "UPDATE " . "tcalendar" . " SET " . substr($tsql,12);
endif;
$dbUpdate = "";
foreach ($arrayoRStcalendar as $fld => $value) {
  $dbUpdate .= $fld . " = " . $value . ",";
}
$dbUpdate = rtrim($dbUpdate,",");
$wherePos = strpos($tsql, " WHERE");
$sql = "UPDATE " . "tcalendar" . " SET " . $dbUpdate . substr($tsql, $wherePos);
...
if($flgMissing == true) {
  $_SESSION["Updatetcalendar_EditFailed"] = 1;
  $_SESSION["SavedEdittcalendarCountryID"] = $_POST["txttcalendarCountryID"];
  $_SESSION["SavedEdittcalendarBranchID"] = $_POST["txttcalendarBranchID"];
  $_SESSION["SavedEdittcalendarDate"] = $_POST["txttcalendarDate"];
  $_SESSION["SavedEdittcalendarDescription"] = $_POST["txttcalendarDescription"];
  $_SESSION["SavedEdittcalendarType"] = $_POST["txttcalendarType"];
}
else {
