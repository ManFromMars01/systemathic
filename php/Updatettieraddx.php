...
$dbValues = "";
    ...
    $rst["CountryID"] = getFormSQLQuoted($objConn1,"ttier","CountryID","txtttierCountryID");
            if (getForm("txtttierBranchID") == ""):
                if($myStatus == ""):
                    $myStatus = "<STRONG>Your insert failed </STRONG><BR><HR>";
                endif;
                $flgMissing = 1;
    ...
    $rst["BranchID"] = getFormSQLQuoted($objConn1,"ttier","BranchID","txtttierBranchID");
            if (getForm("txtttierID") == ""):
                if($myStatus == ""):
                    $myStatus = "<STRONG>Your insert failed </STRONG><BR><HR>";
                endif;
                $flgMissing = 1;
    ...
    $rst["ID"] = getFormSQLQuoted($objConn1,"ttier","ID","txtttierID");
            if (getForm("txtttierDescription") == ""):
                if($myStatus == ""):
                    $myStatus = "<STRONG>Your insert failed </STRONG><BR><HR>";
                endif;
                $flgMissing = 1;
    ...
    $rst["Description"] = getFormSQLQuoted($objConn1,"ttier","Description","txtttierDescription");
...
$sql = "insert into ttier (" . $dbColumns . ") values (" . $dbValues . ")";


if($flgMissing == false):
  $oRSttier = $objConn1->Execute($sql);

  if (!isset($oRSttier) || $oRSttier == false || $oRSttier == ""):
...
if($flgMissing == true) {
  $_SESSION["Updatettier_InsertFailed"] = 1;
  $_SESSION["SavedttierCountryID"] = $_POST["txtttierCountryID"];
  $_SESSION["SavedttierBranchID"] = $_POST["txtttierBranchID"];
  $_SESSION["SavedttierID"] = $_POST["txtttierID"];
  $_SESSION["SavedttierDescription"] = $_POST["txtttierDescription"];
}
else {
  $_SESSION["Updatettier_InsertFailed"] = 0;
}
