...
$sql = "SELECT tunitmeas.CountryID, tunitmeas.BranchID, tunitmeas.ID, tunitmeas.Description  FROM  tunitmeas WHERE  tunitmeas.CountryID = '" . $ID1 . "'" . " AND tunitmeas.BranchID = '" . $ID2 . "'" . " AND tunitmeas.ID = '" . $ID3 . "'";
$oRStunitmeas = $objConn1->SelectLimit($sql,1);
$myStatus = "";
$flgMissing = false;
$arrayoRStunitmeas = array();
if (!$oRStunitmeas):
    $oRStunitmeas->Close();
    $NoRecords = TRUE;
...
$arrayoRStunitmeas["CountryID"] = getFormSQLQuoted($objConn1, "tunitmeas", "CountryID", "txttunitmeasCountryID");
...
$arrayoRStunitmeas["BranchID"] = getFormSQLQuoted($objConn1, "tunitmeas", "BranchID", "txttunitmeasBranchID");
        if (getRequest("txttunitmeasID") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>ID:</strong> : Required field <hr>\n";
        endif;
$arrayoRStunitmeas["ID"] = getFormSQLQuoted($objConn1, "tunitmeas", "ID", "txttunitmeasID");
$arrayoRStunitmeas["Description"] = getFormSQLQuoted($objConn1, "tunitmeas", "Description", "txttunitmeasDescription");
$tsql = $objConn1->GetUpdateSQL($oRStunitmeas, $arrayoRStunitmeas, true, get_magic_quotes_gpc());
$badsql = strpos($tsql, "UPDATE  SET");
if($badsql === false):
    $sql = $tsql;
else:
    $sql = "UPDATE " . "tunitmeas" . " SET " . substr($tsql,12);
endif;
$dbUpdate = "";
foreach ($arrayoRStunitmeas as $fld => $value) {
  $dbUpdate .= $fld . " = " . $value . ",";
}
$dbUpdate = rtrim($dbUpdate,",");
$wherePos = strpos($tsql, " WHERE");
$sql = "UPDATE " . "tunitmeas" . " SET " . $dbUpdate . substr($tsql, $wherePos);
...
if($flgMissing == true) {
  $_SESSION["Updatetunitmeas_EditFailed"] = 1;
  $_SESSION["SavedEdittunitmeasCountryID"] = $_POST["txttunitmeasCountryID"];
  $_SESSION["SavedEdittunitmeasBranchID"] = $_POST["txttunitmeasBranchID"];
  $_SESSION["SavedEdittunitmeasID"] = $_POST["txttunitmeasID"];
  $_SESSION["SavedEdittunitmeasDescription"] = $_POST["txttunitmeasDescription"];
}
else {
