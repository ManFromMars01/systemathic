...
$sql = "SELECT ttier.CountryID, ttier.BranchID, ttier.ID, ttier.Description  FROM  ttier WHERE  ttier.CountryID = '" . $ID1 . "'" . " AND ttier.BranchID = '" . $ID2 . "'" . " AND ttier.ID = " . $ID3;
$oRSttier = $objConn1->SelectLimit($sql,1);
$myStatus = "";
$flgMissing = false;
$arrayoRSttier = array();
if (!$oRSttier):
...
$arrayoRSttier["CountryID"] = getFormSQLQuoted($objConn1, "ttier", "CountryID", "txtttierCountryID");
...
$arrayoRSttier["BranchID"] = getFormSQLQuoted($objConn1, "ttier", "BranchID", "txtttierBranchID");
        if (getRequest("txtttierID") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>ID:</strong> : Required field <hr>\n";
        endif;
$arrayoRSttier["ID"] = getFormSQLQuoted($objConn1, "ttier", "ID", "txtttierID");
        if (getRequest("txtttierDescription") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>Description:</strong> : Required field <hr>\n";
        endif;
$arrayoRSttier["Description"] = getFormSQLQuoted($objConn1, "ttier", "Description", "txtttierDescription");
$tsql = $objConn1->GetUpdateSQL($oRSttier, $arrayoRSttier, true, get_magic_quotes_gpc());
$badsql = strpos($tsql, "UPDATE  SET");
if($badsql === false):
    $sql = $tsql;
else:
    $sql = "UPDATE " . "ttier" . " SET " . substr($tsql,12);
endif;
$dbUpdate = "";
foreach ($arrayoRSttier as $fld => $value) {
  $dbUpdate .= $fld . " = " . $value . ",";
}
$dbUpdate = rtrim($dbUpdate,",");
$wherePos = strpos($tsql, " WHERE");
$sql = "UPDATE " . "ttier" . " SET " . $dbUpdate . substr($tsql, $wherePos);
...
if($flgMissing == true) {
  $_SESSION["Updatettier_EditFailed"] = 1;
  $_SESSION["SavedEditttierCountryID"] = $_POST["txtttierCountryID"];
  $_SESSION["SavedEditttierBranchID"] = $_POST["txtttierBranchID"];
  $_SESSION["SavedEditttierID"] = $_POST["txtttierID"];
  $_SESSION["SavedEditttierDescription"] = $_POST["txtttierDescription"];
}
else {
  $_SESSION["Updatettier_EditFailed"] = 0;
}
