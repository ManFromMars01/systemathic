...
$myStatus = "";
$flgMissing = false;
$arrayoRStmanufacturer = array();
if (!$oRStmanufacturer):
    $oRStmanufacturer->Close();
    $NoRecords = TRUE;
    $myStatus = "The requested record could not be found";
endif;
        if (getRequest("txttmanufacturerCountryID") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                    $myStatus .= " <strong>Country ID:</strong> : Must be in file ";      
                    $myStatus .= "<hr>\n";
        endif;
$arrayoRStmanufacturer["CountryID"] = getFormSQLQuoted($objConn1, "tmanufacturer", "CountryID", "txttmanufacturerCountryID");
        if (getRequest("txttmanufacturerBranchID") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                    $myStatus .= " <strong>Branch ID:</strong> : Must be in file ";      
                    $myStatus .= "<hr>\n";
        endif;
$arrayoRStmanufacturer["BranchID"] = getFormSQLQuoted($objConn1, "tmanufacturer", "BranchID", "txttmanufacturerBranchID");
        if (getRequest("txttmanufacturerID") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>ID:</strong> : Required field <hr>\n";
        endif;
$arrayoRStmanufacturer["ID"] = getFormSQLQuoted($objConn1, "tmanufacturer", "ID", "txttmanufacturerID");
        if (getRequest("txttmanufacturerDescription") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>Description:</strong> : Required field <hr>\n";
        endif;
$arrayoRStmanufacturer["Description"] = getFormSQLQuoted($objConn1, "tmanufacturer", "Description", "txttmanufacturerDescription");
$tsql = $objConn1->GetUpdateSQL($oRStmanufacturer, $arrayoRStmanufacturer, true, get_magic_quotes_gpc());
$badsql = strpos($tsql, "UPDATE  SET");
if($badsql === false):
    $sql = $tsql;
else:
    $sql = "UPDATE " . "tmanufacturer" . " SET " . substr($tsql,12);
endif;
$dbUpdate = "";
foreach ($arrayoRStmanufacturer as $fld => $value) {
  $dbUpdate .= $fld . " = " . $value . ",";
}
$dbUpdate = rtrim($dbUpdate,",");
$wherePos = strpos($tsql, " WHERE");
$sql = "UPDATE " . "tmanufacturer" . " SET " . $dbUpdate . substr($tsql, $wherePos);
...
if($flgMissing == true) {
  $_SESSION["Updatetmanufacturer_EditFailed"] = 1;
  $_SESSION["SavedEdittmanufacturerCountryID"] = $_POST["txttmanufacturerCountryID"];
  $_SESSION["SavedEdittmanufacturerBranchID"] = $_POST["txttmanufacturerBranchID"];
  $_SESSION["SavedEdittmanufacturerID"] = $_POST["txttmanufacturerID"];
  $_SESSION["SavedEdittmanufacturerDescription"] = $_POST["txttmanufacturerDescription"];
}
else {
