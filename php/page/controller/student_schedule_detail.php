...
$studfile = $model->select_where('eattdtl',array('CustNo' => $cust_no));

$teacherID = $studfile->fields['TeacherID'];

$teachertable = $model->select_where('tteacher', array('ID' => $teacherID));

foreach ($teachertable as $teacher ) {
  echo $tname = $teacher['Name'];
}
...
//initialize the external events for calender
  ...
  $('#calendar').fullCalendar({
    ...
    events: [
          ...
          <?php
          foreach($studfile as $ssched): 
        
          ?>
           {
                  title: ' <?php echo $ssched['TimeFrom']; ?> -  <?php echo $ssched['TimeTo']; ?>  ',
                  allDay:false,
                  start: '<?php echo $ssched['Date']; ?> <?php echo $ssched['TimeFrom']; ?>',
                  end: '<?php echo $ssched['Date']; ?> <?php echo $ssched['TimeTo']; ?>' ,
                  description: 'fsdfds',
                  className:'<?php echo str_replace(" ", "",$model->yourlevel($ssched["LevelID"])); ?>'
              },  
