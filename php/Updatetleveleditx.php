...
$arrayoRStlevel["CountryID"] = getFormSQLQuoted($objConn1, "tlevel", "CountryID", "txttlevelCountryID");
...
if($badsql === false):
    ...
    $sql = "UPDATE " . "tlevel" . " SET " . substr($tsql,12);
endif;
$dbUpdate = "";
foreach ($arrayoRStlevel as $fld => $value) {
  $dbUpdate .= $fld . " = " . $value . ",";
}
$dbUpdate = rtrim($dbUpdate,",");
$wherePos = strpos($tsql, " WHERE");
$sql = "UPDATE " . "tlevel" . " SET " . $dbUpdate . substr($tsql, $wherePos);
...
if($flgMissing == true) {
  $_SESSION["Updatetlevel_EditFailed"] = 1;
  $_SESSION["SavedEdittlevelCountryID"] = $_POST["txttlevelCountryID"];
  $_SESSION["SavedEdittlevelBranchID"] = $_POST["txttlevelBranchID"];
  $_SESSION["SavedEdittlevelID"] = $_POST["txttlevelID"];
  $_SESSION["SavedEdittlevelDescription"] = $_POST["txttlevelDescription"];
}
else {
