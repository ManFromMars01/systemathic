...
$sql = "SELECT tseminar.CountryID, tseminar.BranchID, tseminar.ID, tseminar.Description  FROM  tseminar WHERE  tseminar.CountryID = '" . $ID1 . "'" . " AND tseminar.BranchID = '" . $ID2 . "'" . " AND tseminar.ID = " . $ID3;
$oRStseminar = $objConn1->SelectLimit($sql,1);
$myStatus = "";
$flgMissing = false;
$arrayoRStseminar = array();
if (!$oRStseminar):
    $oRStseminar->Close();
    $NoRecords = TRUE;
...
$arrayoRStseminar["CountryID"] = getFormSQLQuoted($objConn1, "tseminar", "CountryID", "txttseminarCountryID");
...
$arrayoRStseminar["BranchID"] = getFormSQLQuoted($objConn1, "tseminar", "BranchID", "txttseminarBranchID");
        if (getRequest("txttseminarID") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>ID:</strong> : Required field <hr>\n";
        endif;
$arrayoRStseminar["ID"] = getFormSQLQuoted($objConn1, "tseminar", "ID", "txttseminarID");
        if (getRequest("txttseminarDescription") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>Description:</strong> : Required field <hr>\n";
        endif;
$arrayoRStseminar["Description"] = getFormSQLQuoted($objConn1, "tseminar", "Description", "txttseminarDescription");
$tsql = $objConn1->GetUpdateSQL($oRStseminar, $arrayoRStseminar, true, get_magic_quotes_gpc());
$badsql = strpos($tsql, "UPDATE  SET");
if($badsql === false):
    $sql = $tsql;
else:
    $sql = "UPDATE " . "tseminar" . " SET " . substr($tsql,12);
endif;
$dbUpdate = "";
foreach ($arrayoRStseminar as $fld => $value) {
  $dbUpdate .= $fld . " = " . $value . ",";
}
$dbUpdate = rtrim($dbUpdate,",");
$wherePos = strpos($tsql, " WHERE");
$sql = "UPDATE " . "tseminar" . " SET " . $dbUpdate . substr($tsql, $wherePos);
...
if($flgMissing == true) {
  $_SESSION["Updatetseminar_EditFailed"] = 1;
  $_SESSION["SavedEdittseminarCountryID"] = $_POST["txttseminarCountryID"];
  $_SESSION["SavedEdittseminarBranchID"] = $_POST["txttseminarBranchID"];
  $_SESSION["SavedEdittseminarID"] = $_POST["txttseminarID"];
  $_SESSION["SavedEdittseminarDescription"] = $_POST["txttseminarDescription"];
}
else {
  $_SESSION["Updatetseminar_EditFailed"] = 0;
}
