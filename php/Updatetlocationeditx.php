...
$sql = "SELECT tlocation.CountryID, tlocation.BranchID, tlocation.ID, tlocation.Description  FROM  tlocation WHERE  tlocation.CountryID = '" . $ID1 . "'" . " AND tlocation.BranchID = '" . $ID2 . "'" . " AND tlocation.ID = " . $ID3;
$oRStlocation = $objConn1->SelectLimit($sql,1);
$myStatus = "";
$flgMissing = false;
$arrayoRStlocation = array();
if (!$oRStlocation):
    $oRStlocation->Close();
    $NoRecords = TRUE;
...
$arrayoRStlocation["CountryID"] = getFormSQLQuoted($objConn1, "tlocation", "CountryID", "txttlocationCountryID");
...
$arrayoRStlocation["BranchID"] = getFormSQLQuoted($objConn1, "tlocation", "BranchID", "txttlocationBranchID");
        if (getRequest("txttlocationID") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>ID:</strong> : Required field <hr>\n";
        endif;
$arrayoRStlocation["ID"] = getFormSQLQuoted($objConn1, "tlocation", "ID", "txttlocationID");
        if (getRequest("txttlocationDescription") == ""):
            $myStatus .= "<STRONG>Some data was missing</STRONG><BR><HR>";
            $flgMissing = TRUE;
                        $myStatus .= " <strong>Description:</strong> : Required field <hr>\n";
        endif;
$arrayoRStlocation["Description"] = getFormSQLQuoted($objConn1, "tlocation", "Description", "txttlocationDescription");
$tsql = $objConn1->GetUpdateSQL($oRStlocation, $arrayoRStlocation, true, get_magic_quotes_gpc());
$badsql = strpos($tsql, "UPDATE  SET");
if($badsql === false):
    $sql = $tsql;
else:
    $sql = "UPDATE " . "tlocation" . " SET " . substr($tsql,12);
endif;
$dbUpdate = "";
foreach ($arrayoRStlocation as $fld => $value) {
  $dbUpdate .= $fld . " = " . $value . ",";
}
$dbUpdate = rtrim($dbUpdate,",");
$wherePos = strpos($tsql, " WHERE");
$sql = "UPDATE " . "tlocation" . " SET " . $dbUpdate . substr($tsql, $wherePos);
...
if($flgMissing == true) {
  $_SESSION["Updatetlocation_EditFailed"] = 1;
  $_SESSION["SavedEdittlocationCountryID"] = $_POST["txttlocationCountryID"];
  $_SESSION["SavedEdittlocationBranchID"] = $_POST["txttlocationBranchID"];
  $_SESSION["SavedEdittlocationID"] = $_POST["txttlocationID"];
  $_SESSION["SavedEdittlocationDescription"] = $_POST["txttlocationDescription"];
}
else {
